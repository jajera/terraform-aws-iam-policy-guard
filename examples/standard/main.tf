# Standard IAM Policy Monitor Example
# This example demonstrates a production-ready deployment with monitoring, alerting, and remediation
...
locals {
  name = "standard-example-${random_string.suffix.result}"
  tags = {
    Environment = "development"
    Project     = "terraform-aws-iam-policy-guard-standard"
    Owner       = "terraform"
    Example     = "standard"
  }
}
...
# Standard deployment with production-ready features
module "iam_policy_monitor" {
  ...
  lambda_memory          = 256 # Standard functions
